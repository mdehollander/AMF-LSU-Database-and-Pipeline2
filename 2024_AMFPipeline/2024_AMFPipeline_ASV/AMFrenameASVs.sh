...
# It takes the ASV_seqs.fasta and ASV_table.tsv output from ampliseq stored in the dada2 directory
...
awk '/^>/{printf ">ASV_" ++i "_" substr($0,2) "\n"; next} 1' ../../../ampliseq_out/dada2/ASV_seqs.fasta  > q2files/ASV_seqs.fasta

# Also adjust the ASV_ID column in the table. Not the best approach, a second cut command is needed. But it works:
awk 'NR==1 {print "ASV_ID\t" $0; next} {print "ASV_" NR-1 "_" $1 "\t" $0}' ../../../ampliseq_out/dada2/ASV_table.tsv | cut --complement -f2 > q2files/ASV_table.tsv
